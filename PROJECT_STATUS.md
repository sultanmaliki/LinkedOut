...
- Row Level Security enabled on all 35 `public` schema tables (default-deny, no policies needed — the API's own connection uses the `postgres` role, which has `rolbypassrls`, so this is pure defense-in-depth against a Supabase anon/service key ever leaking; verified via Supabase's own linter and a live functional check after enabling)
- `apps/web/src/lib/json-ld.ts`'s `toJsonLd()` escapes `<` before embedding data into JSON-LD `<script>` tags — fixes a stored XSS in the profile/company page schema markup (user-controlled bio/description could break out of the script tag); all 3 `dangerouslySetInnerHTML` call sites in the app use it
- `devResetToken`/`devVerificationToken` (dev-mode auth-response shortcuts) require an explicit `ALLOW_DEV_AUTH_TOKENS=true` opt-in, not `NODE_ENV` — closes a account-takeover primitive that was live in production (see [docs/security.md](docs/security.md) for the full incident writeup on both of the above)
- Log hygiene: pino's redact list now also covers `newPassword`/`currentPassword`; `MailerService`'s no-mailer-configured fallback no longer logs the recipient email + raw verification/reset token unless `ALLOW_DEV_AUTH_TOKENS=true` (same opt-in as the line above) — otherwise it logs a generic warning with neither value in it
...
- Access/refresh tokens live in `localStorage`, not `httpOnly` cookies — a deliberate tradeoff (see [docs/security.md](docs/security.md)) that turns any future XSS into full session theft. Fixing it properly means adding CSRF protection and reworking cross-origin cookie config between `api.raifkarani.in` and `linkedout.raifkarani.in`, so it's a planned project, not a quick patch — tracked here, not yet scheduled.
