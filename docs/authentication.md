...
1. `POST /auth/register` — creates a `users` row + a `professionalProfiles` row in one transaction, hashes the password with bcrypt, issues tokens (the account is usable immediately, like most consumer apps), and sends a verification email via `MailerService`. When `RESEND_API_KEY` is set, this is a real email through Resend; otherwise (local dev/test default) the link is logged to the console instead. When `ALLOW_DEV_AUTH_TOKENS=true` (local dev/CI only — see `.env.example`), the token is also returned directly in the response as `devVerificationToken`.
...
7. `POST /auth/forgot-password` — looks up the account by email and, if found and active, emails a password-reset link via `MailerService.sendPasswordResetEmail`. Always responds `{ sent: true }` regardless of whether the email is registered (plus `devResetToken` when `ALLOW_DEV_AUTH_TOKENS=true`), so the endpoint can't be used to enumerate which emails have accounts. `devResetToken`/`devVerificationToken` are gated on that explicit flag, not `NODE_ENV` — a leaked reset token is a full account takeover with no mailbox access needed, and nothing in this repo's Dockerfile/docker-compose pins `NODE_ENV` to `'production'`, so gating on it alone would fail open in any deployment where it's unset or misconfigured. This was a real, live vulnerability in production until caught by a security review — see `docs/security.md`.
